...
export const DormDetails = () => {
    ...
    const fetchDorm = async () => {
      try {
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL}/dorm/${id}`
        );

        if (res.data) {
          // ✅ added safety check only
  ...
  const handleFavorite = async () => {
    ...
    try {
      await axios.post(
        `${process.env.REACT_APP_API_URL}/addFavorite`,
        {
          user_id: user.id,
          dorm_id: dorm.id,
        }
    ...
    <div className="dorm-details-page">
      ...
      <div className="dorm-info">
        ...
        <div className="info-column">
          ...
          <p>
            <strong>Distance to university (min):</strong>{" "}
        ...
        <div className="features-container">
          {dorm.feature1 && (
            <button className="feature-item">{dorm.feature1}</button>
          )}
          {dorm.feature2 && (
            <button className="feature-item">{dorm.feature2}</button>
          )}
          {dorm.feature3 && (
            <button className="feature-item">{dorm.feature3}</button>
          )}
        </div>
      </div>
        ...
        >
          ...
          <button
            ...
            onClick={e => {
              e.stopPropagation();
              setPhotoIndex(
                (photoIndex - 1 + dorm.images.length) %
                  dorm.images.length
              );
            }}
          >
