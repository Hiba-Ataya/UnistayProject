...
export const DormCard = ({ dorm }) => {
  ...
  return (
    ...
    >
      <div
        id={`carousel-${dorm.id}`} // Correct template literal
      ...
      >
        <div className="carousel-inner">
          {dorm.images && dorm.images.length > 0 ? (
            dorm.images.map((img, index) => (
              <div
                key={index}
                className={`carousel-item ${index === 0 ? "active" : ""}`} // Correct template literal
              >
                <img
                  src={`data:image/jpeg;base64,${img}`} // Correct template literal
                  className="d-block w-100"
                  alt={`${dorm.name} ${index + 1}`} // Correct template literal
                />
              </div>
            ))
          ) : (
        ...
        <button
          ...
          type="button"
          data-bs-target={`#carousel-${dorm.id}`} // Correct template literal
          data-bs-slide="prev"
        >
